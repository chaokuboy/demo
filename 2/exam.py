            ...
            with colq:
                if st.button("第一题(问疼痛)",type="secondary",key="1"):
                     with col2:
                         with st.container(height=320, border=True):
                            st.write("**错题分析**")
                            st.write("<h6 style='color: blue;'>第一题、气滞证的疼痛特点是(  )</h6>",unsafe_allow_html=True)
                ...
                if st.button("第十三题(脉数)",type="primary",key="4"):
                     with col2:
                         with st.container(height=320, border=True):
                            st.write("**错题分析**")
                            st.write("<h6>第十三题、病人头晕目眩，少气懒言，倦怠乏力，腹泻，腹部坠胀，脱肛，舌淡苔白，脉弱，属于下列哪一证候(   )</h6>",unsafe_allow_html=True)
                            colx, coly = st.columns(2)
                            with colx:
                                st.write("<h6 style='color: red;'>A. 气虚证</h6>",unsafe_allow_html=True)
                                st.write("<h6>C. 气逆证</h6>",unsafe_allow_html=True)
                                st.write("<h6>正确答案 D</h6>",unsafe_allow_html=True)
                            with coly:
                                st.write("<h6>B. 气血两虚证</h6>",unsafe_allow_html=True)
                                st.write("<h6>D. 气陷证</h6>",unsafe_allow_html=True)
            ...
            with colw:
                ...
                if st.button("第十八题(气虚)",type="primary",key="10"):
                     with col2:
                         with st.container(height=320, border=True):
                            st.write("**错题分析**")
                            st.write("<h6>第十八题、病人胸胁胀闷或走窜疼痛，性情急躁，胁下痞块，刺痛拒按，入夜甚，舌紫暗或有 瘀斑，脉弦涩，属于下列哪一证候(   )</h6>",unsafe_allow_html=True)
                            colx, coly = st.columns(2)
                            with colx:
                                st.write("<h6>A. 气滞证</h6>",unsafe_allow_html=True)
                                st.write("<h6 style='color: red;'>C. 气虚血瘀证</h6>",unsafe_allow_html=True)
                                st.write("<h6>正确答案  B</h6>",unsafe_allow_html=True)
                            with coly:
                                st.write("<h6>B. 气滞血瘀证</h6>",unsafe_allow_html=True)
                                st.write("<h6>D. 血瘀证</h6>",unsafe_allow_html=True)
